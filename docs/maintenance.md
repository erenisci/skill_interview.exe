...
| Task Manager shows ~0% GPU during generation              | **Not a bug.** Its default GPU graphs are 3D / Copy / Video, and CUDA uses none of them | Switch a graph to `Cuda`, or trust `nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv`. Measured healthy on the reference machine: 91–95%, ~3.1 GB, ~57 tok/s                             |
| Editor marks imports red while `npm run typecheck` passes | The editor resolved the file against the wrong tsconfig                                 | The root `tsconfig.json` is a solution file (`files: []` plus references) so each file routes to `tsconfig.node.json` or `tsconfig.web.json`. Restart the TS server if it was opened before this landed |
| `Cuda` missing from the Task Manager graph list           | Hardware-accelerated GPU scheduling                                                     | Turn it off in Settings → System → Display → Graphics, then reboot                                                                                                                                      |
| `llama-server.exe` uses CPU at 100% GPU offload           | Expected — it still does HTTP, tokenization and sampling orchestration on the CPU       | Not evidence of CPU inference; check `ollama ps` and VRAM instead                                                                                                                                       |
